...
/**
 * Local rule-based fallback validator if Gemini API is unavailable
 */
export function validateAnswersLocally(
  letter: string,
  ...
  const keys: CategoryKey[] = ['name', 'place', 'animal', 'thing'];
  const categoriesResult: Record<CategoryKey, { valid: boolean; points: number; bonusMatched: boolean; feedback: string }> = {
    name: { valid: false, points: 0, bonusMatched: false, feedback: '' },
    place: { valid: false, points: 0, bonusMatched: false, feedback: '' },
    animal: { valid: false, points: 0, bonusMatched: false, feedback: '' },
    thing: { valid: false, points: 0, bonusMatched: false, feedback: '' },
  };
  ...
  for (const key of keys) {
    ...
    const startsWithLetter = firstChar === targetLetter;
    ...
    // Basic heuristic sanity check (min length 2)
    let isCategoryValid = rawVal.length >= 2;
    let bonusMatched = false;

    // Evaluate Bonus Rule
    if (isCategoryValid) {
      if (bonusChallenge.id === 'long_words' && rawVal.length >= 5) {
        bonusMatched = true;
      } else if (bonusChallenge.id === 'vowel_rich') {
        const vowelCount = (rawVal.match(/[aeiouAEIOU]/g) || []).length;
        if (vowelCount >= 2) bonusMatched = true;
      } else if (bonusChallenge.id === 'edible_thing' && key === 'thing') {
        bonusMatched = true; // Local default optimistic assumption
      } else if (bonusChallenge.id === 'world_place' && key === 'place') {
        bonusMatched = true;
      } else if (bonusChallenge.id === 'wildlife_expert' && key === 'animal') {
        bonusMatched = true;
      } else {
        bonusMatched = true; // general fallback
      }
    }

    const points = isCategoryValid ? (bonusMatched ? 15 : 10) : 0;
    if (isCategoryValid) validCount++;
    if (bonusMatched) bonusMatches++;

    categoriesResult[key] = {
      valid: isCategoryValid,
      points,
      bonusMatched,
      feedback: isCategoryValid
        ? bonusMatched
  ...
  // Speed Bonus calculation (up to 20 pts for completing under 30s)
  const speedBonus = timeTakenSeconds <= 20 ? 20 : timeTakenSeconds <= 35 ? 10 : timeTakenSeconds <= 50 ? 5 : 0;
  const baseScore = Object.values(categoriesResult).reduce((sum, item) => sum + item.points, 0);
  const totalScore = baseScore + speedBonus;
  const bonusChallengeMet = bonusMatches >= 2;
  ...
  return {
    ...
    speedBonus,
    bonusChallengeMet,
    overallFeedback: validCount === 4
      ? 'Perfect score! All 4 categories matched brilliantly!'
      : validCount >= 2
      ? 'Good effort! You filled out multiple categories.'
      : 'Keep practicing! Give it another shot!',
  };
}
